...
- 助手支持 DeepSeek 流式对话。
- 助手能识别一个系统诊断意图，调用一个后端注册的内置诊断工具，并把结果返回给用户。
...
- MCP 作为第二阶段之后的工具接入方式，第一版只实现 Internal Tool Runtime 和一个后端注册的内置诊断工具。
...
```json
{
  "current_intent": "system_check_status",
  "stage": "answered",
  "entities": {},
...
- system_check_status：系统状态诊断工具意图。
- unknown：无法判断。
...
```json
{
  ...
  "confidence": 0.92,
  "requires_tool": true,
  "candidate_tools": ["system.check_status"]
}
```
...
```json
{
  ...
  "version": "1.0.0",
  "type": "internal",
  "description": "检查 Auraxis runtime 的 gateway 和 database 诊断状态。",
  "risk_level": "diagnostic",
  "enabled": true,
  "required_permissions": ["tool:system.check_status"],
  "timeout_ms": 5000,
...
内置诊断工具用于验证“识别意图 -> 执行工具 -> 返回结果”的闭环。后续如果增加 script tool，仍不能变成任意命令执行器。
...
用户：“帮我检查一下助手服务状态”
  ↓
Router: intent=system_check_status, candidate_tools=["system.check_status"]
  ↓
Policy Gate: permission ok, risk diagnostic
  ↓
Tool Runtime: 执行 tools.ts 中注册的 system.check_status executor
  ↓
ToolCall: succeeded
  ↓
...
type ModelProvider = {
  getProfile(task: ModelTask): { provider: 'deepseek'; model: string }
  streamChat(input: {
    task?: ModelTask
...
- `DEEPSEEK_BASE_URL`
- `DEEPSEEK_MODEL`
- `DEEPSEEK_MODEL_ROUTER`
- `DEEPSEEK_MODEL_CHAT`

`agent_traces.payload` 中的模型调用需要记录排障指标：

- `task` / `provider` / `model`：本次模型用途和实际模型。
- `firstDeltaMs`：从开始调用 chat 到收到第一个流式 delta 的耗时；未收到 delta 时为 `null`。
- `deltaCount`：流式 delta 数量。
- `contentLength`：已收到文本长度。
...
- ToolDefinition 注册。
- Policy Gate。
- 一个 `system.check_status` internal diagnostic tool。
- ToolCall 日志。
- trace_id 贯穿请求、模型调用和工具调用。
- 模型 trace 记录 `firstDeltaMs`、`deltaCount`、`contentLength`，用于拆解首 token 延迟和完整输出耗时。
...
7. 实现 Router JSON 意图识别。
   验证方式：“检查助手状态”类问题稳定输出 `system_check_status`。

8. 实现 Tool Runtime 和 `system.check_status` internal tool。
   验证方式：命中意图后执行注册工具，返回结构化结果，ToolCall 入库。

9. 实现 Policy Gate。
   验证方式：缺少 `tool:system.check_status` 权限时工具被拒绝，且不会执行工具。

10. 补 trace 和错误日志。
    验证方式：一次用户请求能串起 conversation_id、message_id、trace_id、tool_call_id；普通聊天的 model trace 能看到 firstDeltaMs、deltaCount 和 contentLength。
...
- 用户提出系统诊断相关问题时，Router 能识别意图。
- 后端只执行注册过的 internal tool，不执行模型生成的任意命令。
- `system.check_status` 的执行结果返回给用户。
...
用户 A: “帮我检查一下助手服务状态”
期望：
1. Router 输出 system_check_status。
2. Policy Gate 允许 system.check_status。
3. `tools.ts` 中的 system.check_status executor 被执行。
4. 用户看到脚本结果。
...
3. 后续 script tool 是运行在 Auraxis Gateway 容器内，还是需要调用宿主系统已有脚本？
